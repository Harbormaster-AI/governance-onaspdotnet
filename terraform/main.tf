...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      DomainModel = "HCIDL Governance Capabilities Model"
      CertificationId = "263f7aeb-41a8-4284-bd69-5571328e9fec"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.49339072649931615"
...
resource "aws_db_subnet_group" "default" {
  name       = "governanceonaspdotnet-db-subnet"
...
#  name        = "governanceonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application governanceonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for governanceonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "governanceonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "governanceonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_governanceonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "governanceonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/governance-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/governanceonaspdotnet theharbormaster/governance-onaspdotnet:latest"
    ]
  }
}
